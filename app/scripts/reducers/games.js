...
export default function games(state = initialState, action = {}) {
	...
	switch (action.type) {
		...
		case GAMES_DELETE_SUCCESSFULL: 
			...
			const games = state.get('games').delete(String(action.result.id));
		...
		case GAMES_ADD_SUCCESSFULL:
			...
			const games = state.get('games').set(String(addedGame.get('id')), addedGame);
		...
		case GAMES_UPDATE_SUCCESSFULL:
			...
			const games = state.get('games').set(String(updatedGame.get('id')), updatedGame);
